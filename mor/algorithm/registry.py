...
class algorithmRegistry:
    ...
    def _resolveBackend(cls, category: str, backendName: str | None) -> str:
        ...
        if category in cls._defaultBackends:
            return cls._defaultBackends[category]
    ...
    def get(cls, category: str, variant: str | None = None, forceOptions: dict | None = None, backendName: str | None = None, **kwargs) -> Any:
        ...
        if forceOptions:
            ...
            if 'variant' in forceOptions:
                variant = forceOptions['variant']
        if variant is None or variant == 'auto':
            variant = selectAlgorithm(category, backendName=backend, **kwargs)
        if category not in cls._algorithms or variant not in cls._algorithms[category]:
            raise ValueError(f"Algorithm '{variant}' not registered in {category} registry.")
        return cls._algorithms[category][variant](backendName=backend, **kwargs)

    @classmethod
    def list(cls, category: str) -> list:
        return list(cls._algorithms.get(category, {}).keys())
